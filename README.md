...
Telegram --> colab_launcher.py
            ...
            ouroboros/               (agent core)
              ...
              llm.py                -- Multi-provider LLM client (DeepSeek, Comet, Gemini)
...
|-----|----------|-----------------|
| `DEEPSEEK_API_KEY` | Yes | [platform.deepseek.com/api_keys](https://platform.deepseek.com/api_keys) |
| `COMET_API_KEY` | Yes | [www.comet.com/settings/api-keys](https://www.comet.com/settings/api-keys) |
...
| `GOOGLE_API_KEY` | No | [makersuite.google.com/app/apikey](https://makersuite.google.com/app/apikey) -- Enables Gemini provider |
...
CFG = {
    ...
    # Models
    "OUROBOROS_MODEL": "deepseek/deepseek-chat",                 # primary LLM (via DeepSeek)
    "OUROBOROS_MODEL_CODE": "deepseek/deepseek-chat",            # code editing
    "OUROBOROS_MODEL_LIGHT": "deepseek/deepseek-chat",           # consciousness + lightweight tasks
    ...
    "OUROBOROS_MODEL_FALLBACK_LIST": "deepseek/deepseek-chat,comet/comet,google/gemini-2.5-pro-preview",
    # Infrastructure
...
| `COMET_API_KEY` | Comet API key for LLM calls |
...
| `OUROBOROS_MODEL` | `deepseek/deepseek-chat` | Primary LLM model (via DeepSeek) |
| `OUROBOROS_MODEL_CODE` | `deepseek/deepseek-chat` | Model for code editing tasks |
| `OUROBOROS_MODEL_LIGHT` | `deepseek/deepseek-chat` | Model for lightweight tasks (dedup, compaction) |
...
| `OUROBOROS_MODEL_FALLBACK_LIST` | `deepseek/deepseek-chat,comet/comet,google/gemini-2.5-pro-preview` | Fallback model chain for empty responses |
...
### v7.0.0 -- Multi-Provider Architecture Overhaul
- **BREAKING: Complete architecture redesign** -- Replaced OpenRouter-only system with multi-provider chain architecture
- **Primary providers**: DeepSeek (default), Comet, Gemini (optional), with OpenRouter disabled
- **Resilient retry system**: Automatic fallback between providers on failures
- **Cost optimization**: DeepSeek as primary provider for cost-effective operations
- **API key independence**: No longer dependent on OpenRouter balance or availability
- **Enhanced reliability**: Multiple fallback paths ensure continuous operation
- **Updated configuration**: New required API keys (DEEPSEEK_API_KEY, COMET_API_KEY)
- **Backward compatibility**: All existing tools and workflows preserved
- **Tests updated**: Fixed test suite to work with new provider architecture
